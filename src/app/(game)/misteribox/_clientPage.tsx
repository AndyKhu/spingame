...
const MisteriBoxClientPage = ({data}:MBGProps) => {
  const isMobile = useMediaQuery(1024)
 
  const [box,setBox] = useState<gameOption[]>(JSON.parse(JSON.stringify(data)))
  ...
  const handleReset = ()=>{
    setDialog(false)
    setBox(JSON.parse(JSON.stringify(data)))
  }
  useEffect(()=>{
    setBox(JSON.parse(JSON.stringify(data)))
  ...
  return (
    <div className="flex justify-center items-center h-screen w-screen overflow-hidden py-0 lg:py-10 bg-game">
      {dialog && <div className="fixed inset-0 z-[1000] flex justify-center items-center">
        ...
        <Card className="relative px-10 py-14 z-[1001] text-center">
          ...
          <p className="text-xl font-bold mt-5">{price?.option}</p>
          <Button className="absolute top-2 right-3" size="sm" variant={"ghost"} onClick={handleReset}>
            <X size={20}></X>
          </Button>
        </Card>
      </div>}
      <div className="flex relative justify-center items-center h-full bg-black/50 p-5 w-full max-w-6xl flex-col rounded-md">
        <div className="absolute top-0 inset-x-0 flex justify-between p-5">
        ...
        <div className="grid grid-cols-[130px_130px] justify-center  lg:grid-cols-[250px_250px_250px_250px] gap-5 w-full">
         {box.map((item,index)=> (
          <div key={index} className="h-[78px] lg:h-[150px] relative cursor-pointer transition-transform duration-100" style={{transform: `translate(${item.x}px,${item.y}px)`}} onClick={()=>handleOpen(item)}>
            ...
            {item.open && <div className="absolute inset-0 flex justify-center items-center z-50"><p className="text-white font-bold text-xl px-2 rounded-md bg-black/70">{dialog?price?.option:item.option}</p></div>}
          </div>
         ))}
        </div>
        <div className="w-full mt-5 p-5 grid grid-cols-2 lg:grid-cols-3 items-center justify-center gap-3 lg:gap-10 flex-col dark text-white">
        ...
        <div className="flex items-center p-2 px-5 rounded-md justify-center bg-white/70 font-medium text-lg text-green-800 space-x-2">
          <HelpCircle/>
...
export default MisteriBoxClientPage;